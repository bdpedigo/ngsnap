...
- **Public API shape and output/upload (PROPOSED 2026-09-23):** the headline public API is single-input, format-mirroring I/O over a `Spec` (renamed from `Style`): `apply(source, spec)` returns the restyled result in the same shape as its input (link→link, JSON→JSON) with **no new class to reason about**; `check(source, spec)` is the read-only counterpart returning a truthy report; `render(source, out, spec=...)` writes a PNG. `spec` may be a `Spec`, a plain dict, or a TOML path (an anonymous spec). `apply` output is **self-contained**: the state is inlined in the URL fragment and ngsnap never uploads a new state or mints a short link. Uploading a restyled state to a state server (short links) is deferred and, if added, would be a separate opt-in operation needing write credentials. Read-side auth for private inputs stays env-only (P10) via an optional `caveclient` dependency (`cave` extra), see TASK-7. Multiple-input batch (`apply_all`/`render_all`, CLI multi-IO) is punted for now; `TemplatedState` and `RenderSession` remain available as lower-level escape hatches but are not advertised. Deferred: the `Style`→`Spec` rename and single-IO surface are not yet implemented in code.
- **Setting groups and extraction (RESOLVED 2026-09-23):** a **setting group** names a subset of a viewer state's properties for the deployment the team uses, so authors can lift the "look" of a curated link and reuse it. Four groups are classified in `ngsnap.groups` (`GROUPS`): **appearance** (presentation flags, background colors, layout, and per-layer visual props like `shader`/`opacity`/`colorSeed`), **sources** (`dimensions`, and per-layer `type`/`source`/local coords), **selection** (`selectedLayer`, and per-layer `visible`/`segments`/`archived`/`pick`), and **camera** (`position`, cross-section/projection scale/depth/orientation). Each JSON key belongs to at most one group; a layer's `name` is the addressing key, not a group member. `Spec.from_state(source, group)` parses the state (TASK-3) and lifts the group into the spec's `template`, keying per-layer props by layer name so it re-applies through the existing templating path (TASK-4/TASK-16) — the extracted TOML is just a spec authored automatically instead of by hand, with **no second merge mechanism**. Because a template deep-merges over the source, extract-then-apply is idempotent by construction. **Unclassified properties** (e.g. `title`, memory limits, tool bindings) are **dropped** from every extract, never silently folded into a group. `Spec.to_toml()`/`to_file()` serialize the extracted spec (via `tomli-w`) so `Spec.from_file` reloads it; empty `template`/`config`/`respects` sections are omitted to keep a single-group spec minimal. Rejected: reusing `respects` to mark the group boundary (that list names what a spec *leaves alone*, a different axis from what a group *captures*).
- **Cache key (RESOLVED 2026-09-23):** `cache_key(source, spec=None)` returns a short SHA-256 hex string (first 32 hex chars) that is a pure function of two canonical JSON inputs: the **applied** viewer state (`spec` merged over the normalized `source`, serialized by `TemplatedState.to_json` with sorted keys and no host prefix) and the spec's canonical JSON (`Spec.to_json`: template + config + respects, sorted). Hashing the *applied* state is deliberate: state elements the spec overrides do not affect the key (the render is identical), while any non-overridden state element or any change to the spec does. This gives the P6/P7 stability guarantees — identical across Python processes and platforms, and identical for links differing only in host prefix, JSON key order, or URL-safe quoting — because only the canonical viewer state is hashed. **Engine version is intentionally excluded:** the key identifies inputs, not the renderer, so a browser or neuroglancer upgrade that changes pixels does not change the key. Callers who need that invalidation namespace their cache by engine version (e.g. prefix the key), matching design.md's rule that the key must not depend on the package version unless the rendering actually changed. Rejected: hashing the *raw* input state (would invalidate on cosmetic changes the spec overrides away) and baking an engine-version component into the key by default (would churn every cache on an upgrade even when pixels are unchanged, and ngsnap cannot know when an upgrade changes pixels).
- **Package name.**
